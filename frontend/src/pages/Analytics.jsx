  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-foreground">Panel de analitica</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          ...
          <p className="text-xs text-muted-foreground mt-1">Historico</p>
        </div>

        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-muted-foreground">Solicitudes abiertas</span>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {overview.totalTickets > 0
              ? `${Math.round((overview.openTickets / overview.totalTickets) * 100)}% del total`
              : 'No hay solicitudes'}
          </p>
        </div>
        ...
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-muted-foreground">Solicitudes cerradas</span>
            <CheckCircle size={20} className="text-chart-1" />
          </div>
          <p className="text-3xl font-bold text-foreground">{overview.closedTickets}</p>
          <p className="text-xs text-muted-foreground mt-1">
            {overview.totalTickets > 0
              ? `${Math.round((overview.closedTickets / overview.totalTickets) * 100)}% resueltas`
              : 'No hay solicitudes'}
          </p>
        </div>
        ...
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-muted-foreground">Resolucion promedio</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        {}
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          <h2 className="text-xl font-bold text-foreground mb-4">Solicitudes por prioridad</h2>
        ...
        {}
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border">
          <h2 className="text-xl font-bold text-foreground mb-4">Solicitudes por estado</h2>
          <div className="space-y-4">
      ...
      {Object.keys(departmentBreakdown).length > 0 && (
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border mb-8">
          <h2 className="text-xl font-bold text-foreground mb-4">Solicitudes por departamento</h2>
      ...
      {ticketsOverTime.length > 0 && (
        <div className="bg-card p-6 rounded-lg shadow-sm border border-border mb-8">
          <h2 className="text-xl font-bold text-foreground mb-4">Solicitudes en el tiempo (ultimos 30 dias)</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-muted border-b-2 border-border">
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-foreground">Asignadas</th>
                  <th className="px-4 py-3 text-left font-semibold text-foreground">Resueltas</th>
                  <th className="px-4 py-3 text-left font-semibold text-foreground">Pendientes</th>
                  <th className="px-4 py-3 text-left font-semibold text-foreground">Resolucion promedio</th>
                  <th className="px-4 py-3 text-left font-semibold text-foreground">Tasa de exito</th>
                </tr>
              </thead>
              <tbody>
